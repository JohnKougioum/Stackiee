<template>
  <div class="h-auto w-full mt-32">
    <div
      class="relative container center--x w-3/4 h-full flex md:flex md:justify-between"
    >
      <div class="w-full h-full lg:w-4/6 bg-white">
        <PostView />
      </div>
      <div class="w-1/5 h-56 bg-white hidden lg:block">
        <div
          class="w-full h-full flex justify-center items-center p-4 text-center"
        >
          <h1 class="text-lg font-medium">
            Found a bug or do you want a feature?
          </h1>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import PostView from "../component/PostView.vue";
export default {
  name: "Post",
  components: {
    PostView,
  },
};
</script>

<style lang="scss">
.container {
  max-width: 1280px;
}
</style>
